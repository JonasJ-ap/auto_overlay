...
def plot_point(img, pt, color):
    cv2.circle(img, [int(round(v))
               for v in pt], radius=3, color=color, thickness=-1)
...
def mask_grabcutbase(tp, w, h, img):
    ...
    cv2.grabCut(img, mask, rect, np.zeros((1, 65)),
...
def mask_kmeans(tp, w, h, img):
    ...
    _, labels, centers = cv2.kmeans(
        pixs, k, None, criteria, 10, cv2.KMEANS_RANDOM_CENTERS)
...
def track_point(img1, img2, p):
    ...
    new_points, status, err = cv2.calcOpticalFlowPyrLK(
        img1, img2, prev_points, None, winSize=win_size)
    ...
    def auto_overlay(self, *, videopath, start_time, crop_x, crop_y, crop_width, crop_height, rect_width, rect_height, frame_period, show_rect):
        ...
        while 1:
            ...
            if self.tp is not None:
                ...
                if show_rect:
                    plot_rect(dummy, self.tp, rect_width,
                              rect_height, color=red)
